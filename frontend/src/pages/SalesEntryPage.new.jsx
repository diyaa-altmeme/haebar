...
export default function SalesEntryPage() {
  ...
  const setField = (k, v) => {
    ...
    if (["cashActual", "cashSystem", "returns"].includes(k)) {
      // Only format if it's a string input, not if it's already formatted
      if (typeof v === 'string' && v !== '') {
        finalVal = fmtNum(parseNum(v));
      }
    }
  ...
  // Computed Values into IQD (Base)
  const cashActual = parseNum(form.cashActual);
  const cashSystemBase = toBase(form.cashSystem);
  const returnsBase = toBase(form.returns);
  const netCashSystem = Math.max(0, cashSystemBase - returnsBase);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setAlert({ type:"danger", msg:"❌ " + (err.response?.data?.message || "فشل حفظ الشفت") });
